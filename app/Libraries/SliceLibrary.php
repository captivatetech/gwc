...
use Config\Slice as SliceCfg;
use CodeIgniter\View\Exceptions\ViewException;
use CodeIgniter\Language\Language;

$dCFG = new \Config\Paths();
$dCFG = $dCFG->viewDirectory;
define('VIEWPATH', $dCFG.'/');

//
// [2022-09-24] comment lines on 362 to 365 and line 174
//
...
class SliceLibrary
{
	...
	function __construct( array $params = [] )
	{
		...
		{
			//	Autoload Helpers
			empty($this->_ci_helpers) OR helper($this->_ci_helpers);
		}
	...
	public function clear()
	{
		...
		$this->slice_ext		= $this->config->slice_ext;
		$this->cache_time		= $this->config->cache_time;
		$this->enable_autoload	= $this->config->enable_autoload;
		$this->locale			= $this->config->language;
		$this->_ci_libraries	= $this->config->libraries;
		$this->_ci_helpers		= $this->config->helpers;
		$this->_data			= array();
	...
	public function exists($filename, $show_error = FALSE)
	{
		...
		else
		{
			if ($show_error === TRUE)
			{
				throw ViewException::forInvalidFile('Sorry! We couldn\'t find the view: '.$view_name.$this->slice_ext);
			}
			else
			{
				return FALSE;
	...
	protected function _compile($template)
	{
		...
		if ($cached_version = $this->cache->get($cache_name))
		{
			...
			$cached_meta = $this->cache->getMetadata($cache_name);
		...
		//	Store in the cache
		$this->cache->save($cache_name, $content, $this->cache_time);
	...
	public function i18n($line, $params = array())
	{
		...
		if ($string !== NULL)
		{
			if ( ! isset($this->_i18n_loaded[$file]) OR $this->_i18n_loaded[$file] !== $this->locale)
			{
				...
				$line = \Config\Services::language($this->locale)->getLine($line, $params);
			}
	...
	/**
	 *  Rewrites Blade @group statement into check group
	 *
	 *  @param    string   $content
	 *  @return   string
	 */
	protected function _compile_group($content)
	{
		$pattern = '/(\s*)@group(\s*\(.*\))/';
		return preg_replace($pattern, '<?php if ($2); ?>', $content);
	}
	 ...
	 *  @return   string
	 */
	protected function _compile_endgroup($content)
	{
		return str_replace('@endsection', '<?php endif ?>', $content);
	}
